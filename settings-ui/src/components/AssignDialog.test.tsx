...
describe("AssignDialog", () => {
  ...
  it("offers the library as a select dropdown for the ordinary MonitorGrid single-wallpaper flow", async () => {
    renderDialog({ monitors: twoMonitors });

    await userEvent.click(screen.getByRole("radio", { name: "Single wallpaper" }));

    const select = screen.getByLabelText("Wallpaper");
  ...
  it("saves whichever wallpaper is picked from the select dropdown", async () => {
    const onSave = vi.fn().mockResolvedValue(true);
    renderDialog({ monitors: twoMonitors, onSave });

    await userEvent.click(screen.getByRole("radio", { name: "Single wallpaper" }));
    await userEvent.selectOptions(screen.getByLabelText("Wallpaper"), "b");
    await userEvent.click(screen.getByRole("button", { name: "Save" }));
